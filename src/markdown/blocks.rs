...
fn walk_dbg(node: tree_sitter::Node, source: &str, inline_parser: &mut Parser, depth: usize) {
    ...
    if node.kind() == "inline" {
        if let Some(inline_tree) = inline_parser.parse(text, None) {
            ...
            walk_dbg_inline(inline_tree.root_node(), text, depth + 2);
        }
    }
    for child in node.children(&mut node.walk()) {
...
#[cfg(test)]
fn walk_dbg_inline(node: tree_sitter::Node, source: &str, depth: usize) {
    let text = &source[node.byte_range()];
    println!(
        "{:indent$}{}: {:?}",
        "",
    ...
    for child in node.children(&mut node.walk()) {
        walk_dbg_inline(child, source, depth + 1);
    }
}

#[cfg(test)]
mod tests {
    use super::{Block, split_headers_and_images};
    use pretty_assertions::assert_eq;

    #[test]
    fn new_style() {
        let blocks = split_headers_and_images(
            r#"
# header *stronk*!
...
"#,
        ...
        assert_eq!(
            blocks,
            vec![
                Block::Header(1, "header *stronk*!".to_owned()),
                Block::Paragraph("blablagraph\n".to_owned()),
                Block::Header(2, "a baby".to_owned()),
            ]
        );
    }
...
"#,
        ...
        assert_eq!(
            blocks,
            vec![
                Block::Header(1, "header".to_owned()),
...
"#
    ...
    #[test]
    fn split_headers_and_images_with_images() {
        let blocks = split_headers_and_images(
            r#"
# header
...
"#,
        ...
        assert_eq!(
            ...
            vec![
                Block::Header(1, "header".to_owned()),
                Block::Paragraph("paragraph\n".to_owned()),
                Block::Image("zherkalo".to_owned(), "./mirror.jpg".to_owned()),
            ]
        );
    }
}
