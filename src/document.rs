...
impl Document {
    ...
    pub fn find_next_cursor<'b, Iter: DoubleEndedIterator<Item = &'b Section>>(
        iter: Iter,
        current: &CursorPointer,
        mode: FindMode,
        target: FindTarget,
    ) -> Option<CursorPointer> {
        let iter = Document::flatten_sections(iter, &mode, &target);
    ...
    fn flatten_sections<'a>(
        iter: impl DoubleEndedIterator<Item = &'a Section>,
        mode: &FindMode,
        target: &FindTarget,
